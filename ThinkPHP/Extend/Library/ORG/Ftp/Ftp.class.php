...
class Ftp {
    ...
    //private $os_version = 0;  //0 for linux , 1 for macos , 2 for windows
	...
	**/
    ...
    public function mkdir($dirname) {
        ...
        if($this->initdata['os_version'] == 0){//for linux
            $nowdir = '/home/'.$this->initdata['username'].'/';
        }
        else if($this->initdata['os_version'] == 1){//for macos
            $nowdir = '~/'; 
        }
        else if($this->initdata['os_version'] == 2){//for windows
            $nowdir =  '/';
        }
        else{
            $this->err_code = 2;
    ...
    public function get($remote, $local){
        ...
        if (@ftp_get($this->link, $local, $remote, $this->mode)) {
            return true;
        } else {
            $this->err_code = 7;
            return false;
